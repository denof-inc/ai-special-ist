...
describe('AIスペシャリスト.com 先行登録ランディングページ', () => {
  ...
  describe('ファーストビュー', () => {
    ...
    it('サブキャッチコピーが表示される', () => {
      render(<EarlyAccessPage />)
      expect(
        screen.getByText('AIのスキルで活躍したいと願うすべての専門家のために。')
      ).toBeInTheDocument()
      expect(
        screen.getByText(
  ...
  describe('問題提起セクション', () => {
    it('セクションタイトルが表示される', () => {
      render(<EarlyAccessPage />)
      expect(
        screen.getByText('腕のいい職人ほど、営業が苦手なように。')
      ).toBeInTheDocument()
      expect(
        screen.getByText('優れたAI専門家が、仕事探しに悩んでいませんか？')
      ).toBeInTheDocument()
    })
  ...
  describe('インタラクション', () => {
    it('CTAボタンにホバーエフェクトがある', () => {
      ...
      // ホバー前のスタイル確認
      const initialStyle = window.getComputedStyle(ctaButton)
